...
TEST_SUBSCRIPTION_TIER_SCHEMA_VERSION: int = 1
E2E_SYNTHETIC_EMAIL_RE = re.compile(r"^e2e\+probe-[a-z0-9-]+@probe\.resemblio\.com$")
...
class SetTestSubscriptionTierBody(BaseModel):
    """Inbound payload for the E2E-only export entitlement hook.

    The endpoint accepts only the disposable email convention generated by
    the Playwright harness and only the ``solo`` tier. It cannot alter a real
    customer's entitlement or exercise paid billing behavior.
    """

    email: str = Field(min_length=3, max_length=320)
    subscription_tier: str = Field(pattern="^solo$")
...
@dataclass(frozen=True)
class TestSubscriptionTierResponse:
    """Acknowledgement for the synthetic export-entitlement mutation."""
...
@router.post("/internal/test/set_subscription_tier")
def test_set_subscription_tier(
    payload: SetTestSubscriptionTierBody,
    session: Session = Depends(get_db),
    x_test_auth: str | None = Header(default=None, alias="X-Test-Auth"),
) -> JSONResponse:
    """Grant ``solo`` export access to one disposable Playwright user.

    This is deliberately narrower than a general admin endpoint. The same
    stacked test-auth gate required for magic-link readback applies, the email
    must match the CI synthetic-address convention, the only accepted tier is
    ``solo``, and the user must already exist. The E2E test calls the existing
    teardown endpoint before and after use, removing this temporary mutation.
    """
    settings = get_settings()
    if not _enabled(settings):
        return _error(403, "test_auth_disabled")
    ...
    normalized_email = payload.email.lower()
    if E2E_SYNTHETIC_EMAIL_RE.fullmatch(normalized_email) is None:
        return _error(422, "synthetic_email_required")

    user = session.execute(
        select(User).where(User.email == normalized_email)
    ).scalar_one_or_none()
    if user is None:
        return _error(404, "synthetic_user_not_found")

    user.subscription_tier = payload.subscription_tier
    session.commit()
    logger.info(
        "test_subscription_tier_set email=%s user_id=%s tier=%s",
        normalized_email,
        user.id,
        payload.subscription_tier,
    )
    response = TestSubscriptionTierResponse(
        schema_version=TEST_SUBSCRIPTION_TIER_SCHEMA_VERSION,
        ok=True,
        email=normalized_email,
        user_id=user.id,
