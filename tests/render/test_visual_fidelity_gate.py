...
def test_linear_font_spec_matches_actual_live_disclosure() -> None:
    """Linear font specs correctly assert the no-captured-font disclosure text.

    Root cause of the linear 'font: no' drift (2026-06-13 gate run):
    The linear alphabet + about-team specs asserted "Default free type system"
    as the expected disclosure text. But when a brand has no captured font
    tokens, render_font_disclosure_html renders "ships no captured brand font"
    (not "Default free type system"). The "Default free type system" string only
    appears in the disclosure when brand_font_first_preference is NOT None (i.e.
    the brand has a token and the registry falls through to the default mapping).
    Linear has no font tokens -> brand_font_first_preference=None -> different path.

    After the fix: the assertion expected_text is updated to "ships no captured
    brand font" which is what the live HTML actually contains.

    Canonical live HTML generated by:
        render_font_disclosure_html(build_font_disclosure_payload({}), "Linear")
    (verified 2026-06-13; stable as long as DEFAULT_FREE_ALTERNATIVE stays Inter)
    """
    # Canonical live HTML for a brand with no captured font tokens.
    live_html = (
        '<aside class="rs-font-attribution" data-rs-class="font-attribution">'
        "<strong>Linear ships no captured brand font.</strong> "
        "Rendered here with "
    ...
    for category in ("alphabet", "about-team"):
        ...
        assert result is True, (
            ...
            f"The spec assertion probably still checks for 'Default free type system' "
            f"(which only appears in the disclosure when the brand has captured font tokens). "
            f"Linear has no captured font tokens, so the live HTML says "
            f"'ships no captured brand font' instead.\n"
